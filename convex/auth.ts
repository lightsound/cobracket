...
// the current Organizer" from ctx.auth (verified against the core's JWKS via
// auth.config.ts), never from client-supplied arguments.
...
// Who the caller is, as the client sees it: the signed-in Organizer's user id,
// or null. The JWT subject is the app user id the core minted the session for.
