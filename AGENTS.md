...
- `coverage-gaps` gates `fallow health --coverage-gaps` and nothing else: the default run and `fallow audit` do not consult it, so it exits 1 while the UI layer has no tests without turning CI red. Read its **listing**, not its percentage: it is a static test-dependency graph, so `src/router.ts` lazy-importing every page made it count `src/pages/Tournament.tsx` as covered the moment any page had a test, while nothing exercised a line of it. The backlog it names today is the app shell and the two toggles (`App`, `Document`, `LocaleToggle`, `ThemeToggle`, `theme-preference`) plus `scripts/lint-theme.ts`.
