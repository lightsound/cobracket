...
- Auth keys (once per deployment, ADR 0003): `bun run auth:keys` while `convex:dev` is running — the `@convex-dev/auth` CLI generates the RS256 key pair and sets `AUTH_PRIVATE_KEY`/`AUTH_JWKS` on the deployment (idempotent; `--force` rotates); deploys fail until they exist. If `convex dev` is already waiting because those variables were missing, restart it after `auth:keys`: that process does not push again for the second variable (Convex 1.46 ignores the env subscription's first snapshot, and the two variables are set in two calls). Headless environments prefix with `CONVEX_AGENT_MODE=anonymous`. Ignore the "Make sure that it contains" file templates the CLI prints: this repo's `convex/auth.ts` and `auth.config.ts` deliberately diverge from the stock scaffold — never overwrite them with it
- Build: `bun run build`
