...
- Test: `bun run test` (Vitest is the single test runner — ADR 0006; do not use `bun test`). Four projects in `vitest.config.ts`, split by environment. **Every test under `src/` fails if it produced a Solid diagnostic** — `src/test-setup.ts` runs each test body inside a `@solidjs/diagnostics` capture with attribution on and asserts the artifact is empty, so all 36 codes (`SILENT_HOLD`, `UNSTABLE_LIST_IDENTITY`, `STRICT_READ_UNTRACKED`, ...) are errors there even though Solid itself grades most of them `warn` and offers no switch to escalate. That is why the `src` project runs on happy-dom rather than node: the `node` condition resolves Solid's *server* build, where writes are inert (`[SERVER_WRITE]`), effects never run and the attribution tables stay empty — a green reactive test proving nothing. It also asserts `expectNoSilentHolds`, because a hold shorter than the engine's console threshold (`holds.infoMs`, 100ms) never becomes a `SILENT_HOLD` diagnostic — measured: a 41ms unacknowledged hold shows as `silent: 1` in the attribution tables and is reported nowhere. `src/diagnostics-gate.test.tsx` proves both halves of the gate (the engine finds a real mistake here, one case per channel; an undeclared finding fails the test) by re-running itself in a child vitest. A test whose *subject* is a finding declares it with `expectDiagnostics(code)` or `expectSilentHold()`, both of which **require** it to appear — never use either to quiet a finding about real code. The capture opens in `beforeEach`, so a test file's module scope is not gated: keep fixtures and one-off setup there, never a render. Tests under `src/` must stay sequential: the diagnostics channel and the attribution engine are process-global, so `test.concurrent` would attribute one test's finding to another — the gate detects the overlap and fails with that explanation rather than misattributing it
...
- Until all three secrets exist, the workflow's `preflight` job skips `deploy` (grey, with a warning annotation naming the missing secrets) instead of failing the run, so a red `main` always means a failed gate or a failed deploy — never "secrets not set yet". Nothing needs re-enabling: the next push after the secrets are added deploys
...
- `coverage-gaps` gates `fallow health --coverage-gaps` and nothing else: the default run and `fallow audit` do not consult it, so it exits 1 while the UI layer has no tests without turning CI red. Treat its list (currently the remaining `src/*.tsx`, `src/pages/`, `scripts/lint-theme.ts` — `src/bracket/BracketView.tsx` and `src/Standings.tsx` have come off it) as the backlog of what a component test would first cover — it is a static test-dependency graph, not line coverage.
