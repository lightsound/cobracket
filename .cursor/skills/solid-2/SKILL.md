...
This repo uses `solid-js@^2.0.0-rc.0` with `@solidjs/web` and `@solidjs/vite-plugin`.
The hard rules live in `.cursor/rules/solid-2.mdc` (auto-attached to TSX files). This skill
adds the mental model, decision tables, and canonical patterns. When an API is in doubt,
verify against the official docs — see [Checking the official docs](#checking-the-official-docs).
...
Solid 2.0 is a breaking rewrite; 1.x answers are wrong here. The banned-API table is in
`.cursor/rules/solid-2.mdc`.
